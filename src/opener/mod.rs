...
const PLUGIN_IDENTIFIER: &str = "com.plugin.advanced_file_manager";
#[cfg(target_os = "ios")]
...
impl<R: Runtime> Opener<R> {
    ...
    pub fn open_url(&self, url: impl Into<String>, with: Option<impl Into<String>>) -> Result<()> {
        let url = url.into();
        let with = with.map(|w| w.into());
        open::open_url(url, with).map_err(Into::into)
    }
    ...
    pub fn open_path(
        ...
        let path = path.into();
        let with = with.map(|w| w.into());
        open::open_path(path, with).map_err(Into::into)
    }
