...
pub fn reveal_item_in_dir<P: AsRef<Path>>(path: P) -> super::Result<()> {
...
///
/// - **Android / iOS:** Unsupported.
pub fn reveal_items_in_dir<I, P>(paths: I) -> super::Result<()>
where
...
mod imp {
    ...
    pub fn reveal_items_in_dir(paths: &[PathBuf]) -> Result<()> {
        if paths.is_empty() {
            return Ok(());
        }
        ...
        for path in paths {
            let parent = path
                .parent()
                .ok_or_else(|| Error::NoParent(path.to_path_buf()))?;
    ...
    impl OwnedItemIdList {
        fn new(path: &Path) -> Result<Self> {
            ...
            if item_id_list.is_null() {
                Err(Error::FailedToConvertPathToItemIdList(
                    path.to_owned(),
                ))
            } else {
                Ok(Self {
...
mod imp {
    ...
    fn reveal_with_filemanager1(
        ...
        let uris: std::result::Result<Vec<_>, _> = paths
            .iter()
            .map(|path| {
                url::Url::from_file_path(path)
                    .map_err(|_| Error::FailedToConvertPathToFileUrl)
            })
            .collect();
        let uris = uris?;
        ...
        trait FileManager1 {
            async fn ShowItems(&self, name: Vec<&str>, arg2: &str) -> Result<()>;
        }
