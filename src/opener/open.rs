...
pub(crate) fn open<P: AsRef<OsStr>, S: AsRef<str>>(path: P, with: Option<S>) -> super::Result<()> {
    match with {
...
///     tauri_plugin_advanced_file_manager::open_url("https://github.com/tauri-apps/tauri", None::<&str>)?;
///     Ok(())
///   });
/// ```
pub fn open_url<P: AsRef<str>, S: AsRef<str>>(url: P, with: Option<S>) -> super::Result<()> {
    let url = url.as_ref();
    open(url, with)
}
...
///     tauri_plugin_advanced_file_manager::open_path("/path/to/file", None::<&str>)?;
///     Ok(())
///   });
/// ```
pub fn open_path<P: AsRef<Path>, S: AsRef<str>>(path: P, with: Option<S>) -> super::Result<()> {
